...
export function userActivityChangeError(input: {
  ...
  if (
    ...
    input.targetEmail.trim().toLowerCase() ===
      input.initialAdminEmail.trim().toLowerCase()
  ) {
  ...
  if (
    ...
    input.activeAdminCount !== undefined &&
...
export function userPermanentDeleteError(input: {
  ...
  if (input.actorUserId === input.targetUserId)
    return "You cannot permanently delete your own account.";
  if (
    input.targetEmail.trim().toLowerCase() ===
    input.initialAdminEmail.trim().toLowerCase()
  )
    return "The initial Admin cannot be permanently deleted.";
  if (input.targetRole === "admin")
    return "Administrator accounts cannot be permanently deleted from this panel.";
  return null;
}
...
export function userRoleChangeError(input: {
  ...
  if (
    ...
    return "The initial Admin cannot be demoted through this endpoint.";
  if (
    input.targetRole === "admin" &&
    ...
    return "The last active Admin cannot be demoted.";
  return null;
}

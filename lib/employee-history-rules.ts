...
export function normalizeEmployeeHistoryFilter(filter: EmployeeHistoryFilter, today: string) {
  if (!isDateKey(today)) throw new Error("Invalid business date.");
  if (filter.fromDate && !isDateKey(filter.fromDate)) throw new Error("Invalid start date.");
  if (filter.toDate && !isDateKey(filter.toDate)) throw new Error("Invalid end date.");
  if (filter.fromDate && filter.toDate && filter.fromDate > filter.toDate) throw new Error("Start date cannot be after end date.");

  const explicitBounds = filter.fromDate || filter.toDate;
  const defaultBounds = filter.range === "last_7_days"
  ...
  return {
    fromDate: filter.fromDate || (explicitBounds ? undefined : defaultBounds.fromDate),
    toDate: filter.toDate || (explicitBounds ? undefined : defaultBounds.toDate),

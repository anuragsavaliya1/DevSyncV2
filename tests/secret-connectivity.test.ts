...
const mongoClient = mongoUri
  ? new MongoClient(mongoUri, { serverSelectionTimeoutMS: 10_000 })
  : null;
...
describe("configured DevSync external credentials", () => {
  it("connects to the supplied MongoDB Atlas database", async () => {
    ...
    const database = mongoClient!.db(
      process.env.MONGODB_DB_NAME || "devsync_v2"
    );
  ...
  it("persists the Firebase-authenticated initial Admin with the Admin role", async () => {
    ...
    const user = await database
      .collection("users")
      .findOne(
        { email: "anurag.xitijinfo@gmail.com" },
        { projection: { email: 1, role: 1, firebaseUid: 1, isActive: 1 } }
      );
